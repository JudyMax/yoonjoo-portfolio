...
export const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    tldr: 'AI를 활용해 항공사별 API 연동 자동화 PoC를 진행했다. 온보딩 리드타임 3개월→2주 단축을 목표로 하며, 화이트레이블 솔루션으로 세일즈 커버리지를 확대하는 방향을 검증했다.',
    ...
    approach: [
      { title: 'In-Out Data 정의 및 검증 기준 수립', desc: '필수 입력값·유효성 검증 기준을 수립하고, AI 생성 결과 검증 기준을 정의했다.' },
      { title: 'RAG 구조화 및 Ground 개선', desc: '항공사별 연동 오류를 유발하는 Edge Case를 정형화하여 RAG 참조 데이터셋으로 정리.' },
      { title: '화이트레이블 표준 템플릿 기획', desc: 'AI 생성 결과물의 Target Output으로 설정할 기본 Layout & Components를 기획.' },
    ],
    flow: { before: '항공사별 수동 코드 분기. 온보딩 3개월. BE 전용 고객군만 커버.', after: 'RAG 기반 AI 자동화로 Edge Case 처리. 목표 온보딩 2주. FE+BE 패키지로 고객군 확대.' },
    ...
    retrospective: 'RAG 참조 데이터 품질이 PoC 성능의 핵심 변수였다.',
